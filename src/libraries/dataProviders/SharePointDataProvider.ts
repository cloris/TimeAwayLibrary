...
export class SharePointDataProvider implements IMyTimeAwayDataProvider {
  ...
  public getTimeAwayListItemEntityTypeFullName(): Promise<string> {
    return this._webPartContext.spHttpClient.
      get(this._webPartContext.pageContext["web"]["absoluteUrl"] + `/_api/web/lists/GetByTitle('${this._listName}')?$select=ListItemEntityTypeFullName`, SPHttpClient.configurations.v1)
      .then((response: SPHttpClientResponse) => {
        return response.json();
      })
      .then((json: any) => {
        const listItemEntityTypeFullName: string = json['ListItemEntityTypeFullName'];
  ...
  private _createItem(batch: SPHttpClientBatch, item: IMyTimeAwayItem): Promise<SPHttpClientResponse> {
    const body: {} = {
      '@data.type': `${this._ListItemEntityTypeFullName}`,
      'Title': 'My Time Away',
  ...
  private _getFilterString(userId: string): string {
    ...
    if (this._period == TimePeriod.Current) {
      ...
      if (this._mormalWeekToggle) {//5 days
        ...
        condition = moment.utc().startOf('week').subtract(1, 'days').format();// last saturday
      }
